...
> **状态**: PR-1/2/3 已合入 main；PR-5 后端已合入；PR-4(多模型) 与各 UI 层留作后续。
...
1. **PR-1 Schema + 实体（仅新增，零行为变化）** — ✅ 已合入（schema v6→v7）
   - `MessageRows` 加列 + `parentId` 普通索引；`Message`/`Topic` 加字段；
     `MessageRole.root`。**CHECK/FK/偏唯一索引未做**（见 §10 偏离 1）。
...
4. **PR-4 多模型上树** — ⏸ 仅数据层就绪，发送/对比 UI 留给用户
   - 读取已能摞平兄弟组；写兄弟组的发送管线 + UI 未做。
...
1. **不加 DB 级 CHECK / 自引用 FK / 偏唯一索引（§3、§8 原计划用 customStatement 补）。**
   原因：SQLite 不能对已存在表 ALTER 加表级 CHECK/FK（需 12 步重建，重而险）；且偏唯一
   索引 `(topicId) WHERE parentId IS NULL` 在「写入路径改造完成前」会与旧写法产生的
   null-parent 新消息冲突。**单根改由「懒建根 + 回填幂等」在应用层保证**。偏唯一索引留到
   分支功能稳定后的清理 PR 再加。
